...
import { BuildWidgetFromCache, BuildWidgetFromIdForLatest, GetWidgetForTree } from "./WidgetBuild";
    ...
    export async function renderImportedWidget(
      ...
      let cacheWidget = await BuildWidgetFromIdForLatest(widgetId);
      let latestWidgetId = cacheWidget.mainId;
      console.log("renderImportedWidget latestWidgetId, DataCache.cache -->", latestWidgetId, DataCache.cache);
      ...
      if (widgetTree.children.length) {
        widgetTree?.children.forEach((childWidget: WidgetTree) => {
          ...
          if (
            childWidgetLibrary?.css?.length ||
            childWidgetLibrary?.js.length
          ) {
      ...
      if (showDocumentation) {
        ...
        widgetDetailOptionsEl.innerHTML = `
        <button class="widget-documentation-btn" widget-id="${
          widgetTree?.origin || widgetTree?.widgetId
        }" class="d-flex align-items-center gap-1" title="Documentation">
